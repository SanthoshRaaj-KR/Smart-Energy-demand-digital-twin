...
class GridEnvironment:
    ...
    def _build_nodes(self):
        self.nodes: Dict[str, RegionNode] = {
            ...
            "DEL": RegionNode(
                node_id="DEL", name="Delhi",
                generation_mw=14000,
                coords=(28.61, 77.20),
                battery=BatteryCell("DEL", capacity=800, charge=600)
            ),

            "MUM": RegionNode(
                node_id="MUM", name="Mumbai",
                generation_mw=16000,
                coords=(19.08, 72.88),
                battery=BatteryCell("MUM", capacity=1000, charge=700)
            ),

            "KOL": RegionNode(
                node_id="KOL", name="Kolkata",
                generation_mw=9000,
                coords=(22.57, 88.36),
                battery=BatteryCell("KOL", capacity=600, charge=200)
            ),

            "CHE": RegionNode(
                node_id="CHE", name="Chennai",
                generation_mw=11000,
                coords=(13.08, 80.27),
                battery=BatteryCell("CHE", capacity=700, charge=150)
            ),

            "NAG": RegionNode(
                node_id="NAG", name="Nagpur",
                generation_mw=4000,   # small local generation
                coords=(21.15, 79.09),
                battery=None          # pure transit hub — no storage
            ),
        }
    ...
    def _build_edges(self):
        ...
        corridors = [
            ...
            ("DEL", "KOL", 1600, 0.035, 3000, 8.0),   # direct express
            ("DEL", "NAG",  900, 0.020, 4000, 4.0),   # shared heavy corridor
            ("DEL", "MUM", 1400, 0.030, 2500, 6.0),   # west corridor
            ("MUM", "NAG",  800, 0.018, 4000, 3.0),   # shared heavy corridor
            ("MUM", "CHE", 1300, 0.028, 2500, 2.0),   # south coast economy
            ("NAG", "KOL", 1100, 0.024, 3500, 3.0),   # east corridor
            ("NAG", "CHE", 1000, 0.022, 3500, 3.0),   # south corridor
            ("KOL", "CHE", 1700, 0.038, 2000, 1.0),   # long south economy
        ]
    ...
    def _build_paths(self):
        ...
        # ── Delhi ↔ Kolkata ──────────────────
        add("DEL_KOL_1", "DEL","KOL", ["DEL","KOL"],           "Direct express — fast, costly")
        add("DEL_KOL_2", "DEL","KOL", ["DEL","NAG","KOL"],     "Via Nagpur — balanced, shared")

        # ── Delhi ↔ Chennai ──────────────────
        add("DEL_CHE_1", "DEL","CHE", ["DEL","NAG","CHE"],     "Via Nagpur — medium")
        add("DEL_CHE_2", "DEL","CHE", ["DEL","MUM","CHE"],     "Via Mumbai — long, cheap")
        add("DEL_CHE_3", "DEL","CHE", ["DEL","KOL","CHE"],     "Via Kolkata — east economy loop")

        # ── Delhi ↔ Mumbai ───────────────────
        add("DEL_MUM_1", "DEL","MUM", ["DEL","MUM"],           "Direct west corridor")
        add("DEL_MUM_2", "DEL","MUM", ["DEL","NAG","MUM"],     "Via Nagpur — shared hub")

        # ── Mumbai ↔ Kolkata ─────────────────
        add("MUM_KOL_1", "MUM","KOL", ["MUM","NAG","KOL"],    "Via Nagpur — main route")
        add("MUM_KOL_2", "MUM","KOL", ["MUM","CHE","KOL"],    "Via Chennai — south loop, slow")

        # ── Mumbai ↔ Chennai ─────────────────
        add("MUM_CHE_1", "MUM","CHE", ["MUM","CHE"],           "Direct coast — cheap economy")
        add("MUM_CHE_2", "MUM","CHE", ["MUM","NAG","CHE"],     "Via Nagpur hub")

        # ── Kolkata ↔ Chennai ────────────────
        add("KOL_CHE_1", "KOL","CHE", ["KOL","CHE"],           "Direct south — economy")
        add("KOL_CHE_2", "KOL","CHE", ["KOL","NAG","CHE"],     "Via Nagpur")

        # ── Reverse directions (reuse same hops reversed) ──
        reverses = [
            ("KOL_DEL_1","KOL","DEL",["KOL","DEL"],           "Direct express reverse"),
            ("KOL_DEL_2","KOL","DEL",["KOL","NAG","DEL"],     "Via Nagpur reverse"),
            ("CHE_DEL_1","CHE","DEL",["CHE","NAG","DEL"],     "Via Nagpur reverse"),
            ("CHE_DEL_2","CHE","DEL",["CHE","MUM","DEL"],     "Via Mumbai reverse"),
            ("MUM_DEL_1","MUM","DEL",["MUM","DEL"],           "Direct reverse"),
            ("KOL_MUM_1","KOL","MUM",["KOL","NAG","MUM"],    "Via Nagpur reverse"),
            ("CHE_MUM_1","CHE","MUM",["CHE","MUM"],           "Direct reverse"),
            ("CHE_KOL_1","CHE","KOL",["CHE","KOL"],           "Direct reverse"),
            ("CHE_KOL_2","CHE","KOL",["CHE","NAG","KOL"],    "Via Nagpur reverse"),
        ]
    ...
    def set_daily_demand(self, demand_override: Optional[Dict[str, float]] = None):
        ...
        base_fractions = {
            "DEL": 1.05,   # Delhi often exceeds capacity → deficit likely
            "MUM": 0.90,   # Mumbai large surplus usually
            "KOL": 1.15,   # Kolkata typically in deficit
            "CHE": 1.10,   # Chennai high demand, some deficit
            "NAG": 0.60,   # Nagpur low local demand
        }
...
if __name__ == "__main__":
    ...
    print(f"\n{'─'*60}")
    print("  Available paths  DEL → CHE:")
    print(f"{'─'*60}")
    for p in env.get_paths_for("DEL", "CHE"):
    ...
    print(f"\n{'─'*60}")
    print("  Simulating flow: DEL → Nagpur → CHE  (1000 MW)")
    print(f"{'─'*60}")
    env.reset_flows()
    path   = env.paths["DEL_CHE_1"]
    ...
    # ── A second flow through overlapping edges (DEL→NAG is now congested) ──
    print(f"\n  Now routing another 2000 MW:  DEL → Nagpur → KOL")
    path2 = env.paths["DEL_KOL_2"]
    try:
    ...
    print("  Battery Tests:")
    print(f"{'─'*60}")
    stored   = env.store_surplus("MUM", 300)
    supplied = env.discharge_deficit("KOL", 500)
    print(f"  Mumbai stored surplus  : {stored:.1f} MWh")
    print(f"  Kolkata drew from cell : {supplied:.1f} MWh")
    print(f"  Mumbai battery after   : {env.nodes['MUM'].battery}")
    print(f"  Kolkata battery after  : {env.nodes['KOL'].battery}")
